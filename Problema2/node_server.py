...
import json
import time
from Motifs import MotifsSearcher
...
class Blockchain:
    ...
    def mine(self):
        ...
        if not self.unconfirmed_transactions:
            return False
        buscador=MotifsSearcher()
        last_block = self.last_block
        first_transaction=[]
        first_transaction.append(self.unconfirmed_transactions.pop(0))
        dna=first_transaction[0]["content"].replace("\r","")
        dna=dna.split("\n")
        BestMotifs=buscador.GreedyMotifSearch(dna, int(first_transaction[0]["kmer"]), len(dna))
        first_transaction[0]["BestMotifs"]=BestMotifs
        first_transaction[0]["Score"]=buscador.Score(BestMotifs)
...
def new_transaction():
    ...
    required_fields = ["author", "content","title","pass","kmer"]
